...
export default async function agentChat(model = "gpt-3.5-turbo") {
  ...
  const prevMessages = Boolean(readStore().dialogues.agentChat.messagesHistory.length)
    if(!prevMessages || true) { //initiate thing
      writeStore(ps => {
        ps.dialogues.agentChat.messagesHistory = [{role: "system", content: systemString}]
        ps.dialogues.agentChat.historyTokens = calcTokens(systemString).toString()
        return ps
      })
    }
...
async function agentCycle(inputString: string, model = "gpt-3.5-turbo", temperature: number = 0) {
  const updateState = async ( completion: AxiosResponse<CreateChatCompletionResponse, any>, messages: ChatCompletionRequestMessage[] ) => {
    ...
    const expense = calculateExpense(prompt_tokens, completion_tokens, model)
      writeStore((ps) => {
        ps.dialogues.agentChat.messagesHistory = messages
        ps.dialogues.agentChat.historyTokens = `${prompt_tokens+completion_tokens}`
  ...
  if(parseInt(readStore().dialogues.agentChat.historyTokens) + calcTokens(inputString) > contextLength[model]) trimMessages() // keeps it within context length
  
  const {messagesHistory: messages, historyTokens} = readStore().dialogues.agentChat
